// ========================================================== 
// UNIFIED MAIN SCRIPT V5 - FINAL & CORRECTED 
        ...
        loadedProjects.forEach(project => { 
            if (project.artifacts && Array.isArray(project.artifacts)) { 
                project.artifacts.forEach(doc => { 
                    extractedDocs.push({ 
                        id: `${project.id}-${doc.name.replace(/\/g, '-')}`, 
                        name: doc.name, 
...
').filter(line => line.trim()); 
        ...
        if (hasActiveFilters) { 
            itemsToFilter = itemsToFilter.filter(p => { 
                return filterKeys.every(key => { 
                    ...
                    const projectValues = [].concat(p[key]).map(v => v.toLowerCase()); 
                    const filterValues = activeFilters[key].map(v => v.toLowerCase()); 

                    return filterValues.some(f => projectValues.includes(f)); 
                }); 
            }); 
        } 

        renderItems(itemsToFilter); 
        updateFilterPanel(itemsToFilter); 
    } 

    function populateFilterPanel(projectsData) { 
        if (!filterGroupContainer) return; 

        const facets = getVisibleFacets(projectsData); 

        const mediumOptions = facets.medium.map(medium => { 
            const key = medium.toLowerCase(); 
            return ` 
                <div class="filter-option"> 
                    <input type="checkbox" id="filter-medium-${key}" name="medium" value="${medium}" checked> 
                    <label for="filter-medium-${key}">${medium} <span class="filter-count">(${facets.mediumCounts[medium] || 0})</span></label> 
                </div> 
            `; 
        }).join(''); 

        const genreOptions = facets.genres.map(genre => ` 
            <div class="filter-option"> 
                <input type="checkbox" id="filter-genre-${genre.replace(/\s+/g, '-')}" name="genre" value="${genre}"> 
                <label for="filter-genre-${genre.replace(/\s+/g, '-')}">${genre} <span class="filter-count">(${facets.genreCounts[genre] || 0})</span></label> 
            </div> 
        `).join(''); 

        const techOptions = facets.techs.map(tech => ` 
            <div class="filter-option"> 
                <input type="checkbox" id="filter-tech-${tech.replace(/\\ W+/g, '-')}" name="tech" value="${tech}"> 
                <label for="filter-tech-${tech.replace(/\\ W+/g, '-')}">${tech} <span class="filter-count">(${facets.techCounts[tech] || 0})</span></label> 
            </div> 
        `).join(''); 

        const styleOptions = facets.styles.map(style => ` 
            <div class="filter-option"> 
                <input type="checkbox" id="filter-style-${style.replace(/\s+/g, '-')}" name="style" value="${style}"> 
                <label for="filter-style-${style.replace(/\s+/g, '-')}">${style} <span class="filter-count">(${facets.styleCounts[style] || 0})</span></label> 
            </div> 
        `).join(''); 
    ...
    function updateFilterPanel(filteredProjects) { 
        const facets = getVisibleFacets(filteredProjects); 
        ...
        filterGroupContainer.querySelectorAll('.filter-option').forEach(option => { 
            ...
            switch (filterType) { 
                case 'medium': 
                    count = facets.mediumCounts[filterValue] || 0; 
                    break; 
                case 'genre': 
                    count = facets.genreCounts[filterValue] || 0; 
                    break; 
                case 'tech': 
                    count = facets.techCounts[filterValue] || 0; 
                    break; 
                case 'style': 
                    count = facets.styleCounts[filterValue] || 0; 
                    break; 
            } 
    ...
    function updateActiveFiltersFromPanel() { 
        ...
        const allMediums = [...new Set(allProjects.map(p => p.medium).flat())]; 
        if (activeFilters.medium.length === allMediums.length || activeFilters.medium.length === 0) { 
            activeFilters.medium = []; 
        } 
    } 

    function getVisibleFacets(projects) { 
        const mediumCounts = {}; 
        const genreCounts = {}; 
        const techCounts = {}; 
        const styleCounts = {}; 
        ...
        projects.forEach(p => { 
            (p.medium ? [p.medium] : []).forEach(m => mediumCounts[m] = (mediumCounts[m] || 0) + 1); 
            (p.genre || []).forEach(g => genreCounts[g] = (genreCounts[g] || 0) + 1); 
            (p.tech || []).forEach(t => techCounts[t] = (techCounts[t] || 0) + 1); 
            (p.style || []).forEach(s => styleCounts[s] = (styleCounts[s] || 0) + 1); 
        }); 

        return { 
            medium: Object.keys(mediumCounts), 
            genres: Object.keys(genreCounts), 
            techs: Object.keys(techCounts), 
...
if (modalTabs.length && tabContents.length) { 
    ...
    // --- Add handlers for closing modal with ESC key, click outside, and close button --- 
    if (connectModal) { 
        // Handle clicks on the modal overlay (for closing) 
        connectModal.addEventListener('click', (e) => { 
            // Close if the overlay background or the close button (or its icon) is clicked 
            if (e.target === connectModal || e.target.closest('.modal-close-btn')) { 
        ...
        document.addEventListener('keydown', (e) => { 
            if (e.key === 'Escape' && connectModal.classList.contains('is-visible')) { 
                connectModal.classList.remove('is-visible'); 
            } 
        }); 
    } 
