...
function highlightSummary(text: string, keyPrefix: string) {
  ...
  return parts.map((part, i) =>
    SUMMARY_HIGHLIGHTS.includes(part) ? (
      <em key={`${keyPrefix}-${i}`} className="font-semibold text-steel">
        {part}
      </em>
    ) : (
      ...
      <div ref={scrollerRef} className="hide-scroll min-h-0 overflow-y-auto">
        <Pane kicker="01 · Summary" title="Architect, builder, product lead">
          <Blueprint className="max-w-5xl p-5">
            <div className="flex max-w-[68ch] flex-col gap-4 text-body-lg leading-relaxed text-mute-800">
              {SUMMARY.split("\n\n").map((para, pi) => (
          ...
          <Blueprint className="max-w-5xl p-5">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2 text-body text-steel-800">
        ...
        <Pane kicker="02 · Expertise" title="Architecture & engineering">
          <div className="grid max-w-5xl gap-4 md:grid-cols-2">
            {EXPERTISE.map((e) => (
              <Blueprint key={e.title} className="flex flex-col gap-2 p-4">
                ...
                <div className="mt-1 font-heading text-h-item font-semibold leading-[1.12] tracking-tight">{e.title}</div>
                <p className="text-body leading-relaxed text-mute-800">{e.items}</p>
              </Blueprint>
            ))}
          </div>
          <div className="mt-2 flex max-w-5xl flex-wrap gap-2">
            {SKILLS.map((s) => (
              <span
                key={s}
                className="border border-line px-3 py-1 text-caption uppercase tracking-wide text-mute-800"
              >
                {s}
              </span>
            ))}
        ...
        <Pane kicker="03 · Selected architecture" title={FEATURED_ARCHITECTURE.title}>
          <Blueprint className="max-w-5xl p-5">
            <div className="text-body text-steel-700">
              {FEATURED_ARCHITECTURE.role} · {FEATURED_ARCHITECTURE.year}
            </div>
            <ul className="mt-3 max-w-[72ch] space-y-3 text-body leading-relaxed text-mute-800">
          ...
          <div className="grid max-w-5xl gap-4">
            {ADDITIONAL_SYSTEMS.map((s) => (
              <Blueprint key={s.title} className="p-5">
                ...
                <h3 className="mt-2 font-heading text-h-item font-semibold leading-[1.12] tracking-tight">{s.title}</h3>
                <ul className="mt-3 space-y-2 text-body leading-relaxed text-mute-800">
                  {s.bullets.map((b) => (
          ...
          <div className="grid max-w-4xl gap-4 md:grid-cols-2">
            {EDUCATION.map((e) => (
              <Blueprint key={e.credential} className="p-5">
                ...
                <p className="mt-2 text-body leading-relaxed text-mute-800">{e.org}</p>
              </Blueprint>
            ))}
          </div>
          <a
...
function Pane({
  ...
  return (
    <article className="flex flex-col justify-start gap-4 border-t border-line px-4 py-8 first:border-t-0 first:pt-4 lg:gap-5 lg:px-8 lg:py-10 lg:first:pt-4 last:pb-20">
      ...
      {title ? (
        <h2 className="max-w-[32ch] font-heading text-h-section font-semibold tracking-tight">
          {title}
        </h2>
      ) : null}
      {children}
